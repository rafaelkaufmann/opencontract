...
* The contract body and the predicates `isExpired`, `isRevoked` are all written as JavaScript. The library supplies a context for (async) execution and some treatment on deserialization, but this is still foreign code being `eval`'d. *Caveat emptor*. Most likely, we will rely on human-verified signed contract templates for trust.
...
* `body : Function<() => Promise<JointState>>` - this defines your "contract body" - that is, under which conditions it should be considered to be valid or breached for each party. More information on `JointState` below. The default is for contract to be considered always valid (this is useful somehow?).
