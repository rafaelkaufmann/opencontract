...
class CompoundClause extends Clause {
    constructor(operator, children) {
        ...
        for (let child of children) {
            ...
            _child.parent = this;
            this.parties = _.union(this.parties, child.parties);
...
class BaseClause extends Clause {
    ...
    async eval () {
        return asyncMapValues(this._statesOrQueries, async (sOrQ) => {
            if (sOrQ instanceof State) {
                return sOrQ;
            } else {
                return await sOrQ.run();
            }
        });
    }

    clearMemo() {
        for (let sOrQ of this._statesOrQueries) {
